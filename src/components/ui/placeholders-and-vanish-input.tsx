...
export function PlaceholdersAndVanishInput({
  ...
  const draw = useCallback(() => {
    ...
    const newData = Array.from({ length: 800 }, (_, t) => {
      return Array.from({ length: 800 }, (_, n) => {
        const i = 4 * t * 800;
        const e = i + 4 * n;
        if (
          pixelData[e] !== 0 &&
  ...
  const animate = useCallback((start: number) => {
    const animateFrame = (pos: number = 0) => {
      requestAnimationFrame(() => {
        newDataRef.current = newDataRef.current.map((current) => {
          if (current.x < pos) return current;
